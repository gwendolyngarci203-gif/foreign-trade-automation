...
const plan = validateManagedPlan({
  ...
  planId: "managed-test",
  mode: "managed",
  hsCodes: [{ hsCode: "000000", startPage: 8 }],
  dailyBudgets: { buyerEntries: 1200, emailSends: 999, globalEmailHardCap: 999 },
});
assert.equal(plan.hsCodes[0].startPage, 8);
assert.equal(plan.dailyBudgets.emailSends, 200);
assert.equal(plan.dailyBudgets.globalEmailHardCap, 200);
assert.equal(plan.dailyBudgets.buyerEntries, 1200);
assert.equal(plan.dailyBudgets.companyDetails, 100);
assert.deepEqual(plan.deliveryPolicy, { maxContactsPerCompanyDaily: 2, validEmailCompaniesDaily: 100, draftContactsPerCompany: 4, dailyDraftTarget: 200, reviewMode: "central_batch" });
...
assert.throws(() => validateManagedPlan({ schemaVersion: 1, mode: "managed", planId: "x", hsCodes: [{ hsCode: "000000" }, { hsCode: "000000" }] }), /重复/);
...
}, { ...plan, dailyBudgets: { ...plan.dailyBudgets, companyDetails: 1 } }, "000000");
...
}, { ...plan, hsCodes: [{ ...plan.hsCodes[0], countries: ["MX"] }] }, "000000", new Set(["Seen Inc."])).map((item) => item.company), ["Fresh Inc."]);
assert.throws(() => selectManagedCompanies({ kind: "netease-customs-discovery", source: { query: "000000" }, safety: { captcha: true }, records: [] }, plan, "000000"), /安全信号/);
