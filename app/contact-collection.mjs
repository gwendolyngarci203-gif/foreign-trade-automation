...
export function createOrSyncQueue(existing, input, now = new Date()) {
  ...
  for (const entry of entries) {
    ...
    const item = {
      ...
      status: processed.has(entry.normalizedName) ? "completed" : "pending",
      attempts: 0,
      leaseOwner: "",
      leaseExpiresAt: null,
      lastSignal: "none",
      ...
      contactRows: 0,
      artifactReference: "",
      completedAt: processed.has(entry.normalizedName) ? at : null,
      updatedAt: at,
    };
...
export function completeBatch(queue, batchId, input, now = new Date()) {
  ...
  for (const result of results) {
    ...
    const signal = COLLECTION_SIGNALS.has(String(result.signal || "none")) ? String(result.signal || "none") : "structure_error";
    const outcome = ["ok", "no_result", "skipped", "error"].includes(result.outcome) ? result.outcome : "error";
    item.lastSignal = signal;
    item.lastError = String(result.error || "").slice(0, 500);
    item.contactRows = integer(result.contactRows, 0, 0, 100000);
