...
function readonlyObservabilitySnapshot() {
  const businessDate = currentBusinessDate();
  const rawMetrics = {
    draftsGenerated: (pipelineStore.jobs || []).reduce((sum, job) => sum + (job.artifacts || [])
      .filter((artifact) => artifact.stage === "drafting" && businessDateFor(new Date(artifact.at)) === businessDate)
      .reduce((count, artifact) => count + Number(artifact.counts?.draftedContacts || 0), 0), 0),
    autoApproved: (pipelineStore.jobs || []).reduce((sum, job) => sum + (job.audit || [])
      .filter((event) => event.type === "pipeline_auto_approved" && businessDateFor(new Date(event.at)) === businessDate)
      .reduce((count, event) => count + Math.max(1, Number(event.draftIndexes?.length || 0)), 0), 0),
    outboxCreated: (outboxStore.entries || []).filter((entry) => businessDateFor(new Date(entry.createdAt)) === businessDate).length,
    accepted: (outboxStore.entries || []).filter((entry) => entry.status === "accepted" && businessDateFor(new Date(entry.updatedAt || entry.createdAt)) === businessDate).length,
    failed: (outboxStore.entries || []).filter((entry) => entry.status === "failed" && businessDateFor(new Date(entry.updatedAt || entry.createdAt)) === businessDate).length,
    retries: (outboxStore.entries || []).reduce((sum, entry) => sum + (entry.events || [])
      .filter((event) => /retry|requeued/.test(String(event.type || "")) && businessDateFor(new Date(event.at)) === businessDate).length, 0),
  };
  const classifiedMetrics = buildClassifiedMetrics({
    jobs: pipelineStore.jobs || [],
    outboxEntries: outboxStore.entries || [],
    feedbackEvents: runtimeStateStore.feedback.events || [],
    businessDate,
    businessDateFor,
  });
  return {
    schemaVersion: OBSERVABILITY_SCHEMA_VERSION,
    classifications: OBSERVABILITY_CLASSIFICATIONS,
    businessDate,
    rawMetrics,
    classifiedMetrics,
    reconciliation: reconcileMetrics(rawMetrics, classifiedMetrics, { total: classifiedMetrics.mappingCount }),
    readOnly: true,
  };
}

function readonlyControlPlaneStatus() {
  const observability = readonlyObservabilitySnapshot();
  return {
    readOnly: true,
    production: observability.classifiedMetrics.production,
    canaryTest: observability.classifiedMetrics.canaryTest,
    recovery: observability.classifiedMetrics.recovery,
    observability,
  };
}
...
async function apiHandlerUnlocked(req, res, url, model) {
  ...
  if (req.method === "GET" && pathname === "/api/observability") {
    return json(res, 200, readonlyObservabilitySnapshot());
  }
  if (req.method === "GET" && pathname === "/api/control-plane/status") {
    return json(res, 200, readonlyControlPlaneStatus());
  }
  ...
  const pipelineMatch = pathname.match(/^\/api\/pipeline\/jobs\/([^/]+)\/(resume|stage|pause|circuit|recover|approve|auto-approve|simulate-send|send)$/);
  if (pipelineMatch && req.method === "POST") {
    ...
    if (action === "auto-approve") {
      if (input.confirm !== `AUTO APPROVE PIPELINE ${id}`) return json(res, 428, { error: `需要输入确认短语：AUTO APPROVE PIPELINE ${id}` });
      if (input.mode !== "auto") return json(res, 422, { error: "自动审核必须显式声明 mode=auto" });
      if (job.currentStage !== "approval" || !["batch_review", "waiting_input"].includes(job.status)) return json(res, 409, { error: "流水线尚未停在自动审核门" });
      const artifact = await readPipelineArtifact(job, "drafting");
      const result = buildApprovalAdapterResult({ job, artifact, context: input.observability || {} });
      const checkedAt = now;
      job.approval = { ...(job.approval || {}), mode: "auto", decision: result.decision, checkedAt };
      job.batchReview = { ...(job.batchReview || {}), status: result.decision.decision === "approved" ? "approved" : "rejected", decision: result.decision };
      job.audit = [...(job.audit || []), { at: checkedAt, ...result.audit }].slice(-1000);
      if (result.decision.decision === "approved") {
        job.approval.approvedAt = checkedAt;
        job.approval.approvedBy = "auto-approval-adapter";
        job.approval.recipientEvidence = [];
        job.stages.approval = { ...(job.stages.approval || {}), status: "completed", updatedAt: checkedAt };
        job.currentStage = "sending";
        job.status = "waiting_input";
        job.requiredInput = "自动审核已通过；发送仍需显式人工操作";
        job.stages.sending = { ...(job.stages.sending || {}), status: "waiting_input", updatedAt: checkedAt };
      }
      job.updatedAt = checkedAt;
      await writeJsonStore(pipelinePath, pipelineStore);
      return json(res, 200, { ok: true, jobId: id, ...result, persisted: true });
