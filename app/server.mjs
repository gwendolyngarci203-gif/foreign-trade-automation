...
const unsubscribeHmacSecretFile = process.env.UNSUBSCRIBE_HMAC_SECRET_FILE || path.join(appDir, "data", "unsubscribe-hmac-secret.txt");
...
const managedPlanPath = process.env.MANAGED_PLAN_PATH || path.join(workspaceDir, "plans", "managed-hscode-plan.example.json");
const managedHistoryReportPath = process.env.MANAGED_HISTORY_REPORT_PATH || path.join(appDir, "data", "managed-history-report.json");
const sourcePath = process.env.SOURCE_PATH || path.join(workspaceDir, "examples", "sample-source.json");

const port = Number(process.env.PORT || 4173);
const host = process.env.HOST || "127.0.0.1";
const MAX_BODY_BYTES = 1024 * 1024;
const OPENAI_BASE_URL = (String(process.env.OPENAI_BASE_URL || "").trim() || "https://api.openai.com/v1").replace(/\/+$/, "");
const OPENAI_MODEL = String(process.env.OPENAI_MODEL || "").trim() || "gpt-5.2";
...
const EMAIL_SCENARIOS = [
  {
    id: "first_touch",
    name: "首次开发信",
    goal: "Use one buyer-specific relevance sentence, introduce one verified capability, and ask one practical question.",
    wordRange: [80, 130],
    maxAssets: 1,
...
const GUARDED_CLAIMS = [
  { id: "verified_experience", label: "已核实经验声明", text: "The sender's stated industry experience has been independently verified.", pattern: /years? of experience/i },
  { id: "verified_client_reference", label: "已授权客户案例", text: "The named client reference is approved for external use.", pattern: /client reference/i },
  { id: "approved_payment_terms", label: "已批准付款条款", text: "The stated payment terms are approved for this prospect.", pattern: /payment terms?/i },
  { id: "verified_lead_time", label: "已核实交期", text: "The stated lead time is verified for the relevant specification and quantity.", pattern: /lead time/i },
];

// Public release deliberately ships without proprietary images. Add only assets
// you own and update this allowlist before enabling image attachments.
const EMAIL_ASSETS = [];
...
const deliveryConfig = {
  ...
  batchLimit: boundedInteger(process.env.SEND_BATCH_LIMIT, 200, 1, 200),
  dailyLimit: boundedInteger(process.env.SEND_DAILY_LIMIT, 200, 1, 200),
  accountDailyLimit: boundedInteger(process.env.SEND_ACCOUNT_DAILY_LIMIT, 20, 1, 20),
...
const DEFAULT_SENDER_PROFILE = {
  ...
  unsubscribeBaseUrl: "",
  unsubscribeReplyMailbox: "unsubscribe@example.com",
...
function mailboxAccountConfig(address) {
  ...
  return {
    ...
    pass,
    from: user,
    fromName: String(senderProfileStore.companyDisplayName || senderProfileStore.companyLegalName || "Your Company").trim(),
    replyTo: user,
  };
}
...
function mailboxSenderPriority(message) {
  ...
  const ownAccount = (mailboxAccountsStore.accounts || []).some((item) => normalize(item.address || item.user) === address);
  return ownAccount
    || /^(?:postmaster|mailer-daemon)@/.test(address)
    || /post\s*master|mailer.?daemon/.test(name)
    ? 1
    : 0;
}
...
const workflow = [
  ...
  { id: "sending", order: 8, name: "SMTP受控发送", status: "complete", progress: 100, output: "服务器端10个账号、每日200封硬上限、防重、限速、熔断和冷却恢复均已验收；受控发送链路已部署" },
...
function cleanTaskInput(input) {
  ...
  if (managed && input.automation?.confirm !== `AUTHORIZE MANAGED ${hsCode}`) {
    throw Object.assign(new Error(`计划托管需要确认短语：AUTHORIZE MANAGED ${hsCode}`), { status: 428 });
  }
  const emailSendsDaily = bounded(input.budgets?.emailSendsDaily, managed ? 200 : 20, 1, 200);
  ...
  return {
    ...
    automation: {
      ...
      planId: managed ? String(input.automation?.planId || `managed-${hsCode}`).replace(/[^a-zA-Z0-9_-]/g, "-").slice(0, 120) : null,
...
async function requestOpenAi(url, options) {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "outreach-openai-"));
...
async function dailyPipelineBatch(limit = deliveryConfig.dailyLimit, reserveMode = false, centralOnly = false) {
  ...
  const companyTarget = Number(managedTask?.budgets?.validEmailCompaniesDaily || 100);
...
function dailyTaskStatus(batch, draftPool) {
  ...
  const circuitOpen = Boolean(
    ...
    || task?.safetyState === "CIRCUIT_OPEN"
    || jobs.some((job) => job.status === "circuit_open")
  ...
  const waitingInput = jobs.some((job) => job.status === "waiting_input" && job.currentStage !== "feedback");
  const sending = activeOutbox || jobs.some((job) => (
    job.currentStage === "sending" || job.stages?.sending?.status === "running"
  ));
  const drafting = jobs.some((job) => (
...
async function sendPipelineDraftBatch(job, drafts, draftIndexes) {
  const results = [];
  for (let index = 0; index < Math.min(drafts.length, deliveryConfig.batchLimit, dailyDeliveryUsage().remaining); index += 1) {
...
async function generateAiDraft(input) {
  ...
  const facts = {
    ...
    sender_company: clipText(input.senderCompany, 200) || "Your Company",
...
async function buildEmailMessage({ to, subject, body, assetIds = [], messageId = "" }, smtpConfig = deliveryConfig) {
  ...
  const images = selectedAssets.map((asset, index) => ({ ...asset, cid: `outreach_asset_${index + 1}` }));
...
async function sendInterventionAlert(input) {
  ...
  const daily = dailyDeliveryUsage();
  const campaign = { id: `ops-alert-${daily.businessDate}-${code}` };
  ...
  try {
    ...
    const delivery = await sendSmtpMessage({
      to: OPS_ALERT_EMAIL,
      subject: `【Automation Alert】${title}`,
      body,
      messageId: entry.messageId,
...
function isStoreMutationRequest(method, pathname) {
  ...
  return /^\/api\/(contact-quality|suppressions|ops\/(?:tasks|intervention-alert)|contact-queues|pipeline|outbox|mailbox|campaigns|sender-profile|feedback|delivery-circuit|delivery-mode|managed-cycle)(?:\/|$)/.test(pathname);
}
...
async function apiHandlerUnlocked(req, res, url, model) {
  ...
  if (pipelineMatch && req.method === "POST") {
    ...
    if (action === "approve") {
      ...
      job.approval = {
        ...
        providerAuthorization: String(input.providerAuthorization || "user_supplied_provider_authorization").slice(0, 300),
    ...
    try {
      ...
      if (activeTask) {
        return json(res, 200, {
          ...
          hsCodes: [{ hsCode: activeTask.hsCode, direction: activeTask.direction, countries: activeTask.countries, startPage: Math.max(1, Number(activeTask.checkpoint?.page || 0) + 1) }],
          dailyBudgets: {
  ...
  if (req.method === "POST" && pathname === "/api/ops/tasks") {
    ...
    if (input.automation.mode === "managed") {
      ...
      for (const prior of (operationsStore.tasks || []).filter((item) => (
        ...
        prior.updatedAt = now;
        prior.audit = [...(prior.audit || []), { at: now, type: "managed_plan_replaced", replacementHsCode: input.hsCode }].slice(-1000);
        for (const job of (pipelineStore.jobs || []).filter((item) => (
          ...
          job.leaseOwner = null;
          job.leaseExpiresAt = null;
          job.requiredInput = `托管HSCode已切换为${input.hsCode}`;
          job.updatedAt = now;
  ...
  if (taskMatch) {
    ...
    if (req.method === "PUT" && !action) {
      ...
      const input = await parseBody(req);
      if (input.confirm !== `AUTHORIZE MANAGED ${task.hsCode}`) return json(res, 428, { error: `需要输入确认短语：AUTHORIZE MANAGED ${task.hsCode}` });
      const emailSendsDaily = boundedInteger(input.budgets?.emailSendsDaily, task.budgets.emailSendsDaily, 1, 200);
...
server.listen(port, host, () => {
  console.log(`Foreign Trade Automation Toolkit running at http://${host}:${port}`);
