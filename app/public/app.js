...
async function loadBase() {
  const [health, summary, quality, workflowData, managedPlan, options, buyers, contacts, contactQuality, suppressions, operations, contactQueues, pipeline, dailyBatch, inventory, outbox, mailbox, campaigns, senderProfile, deliveryMode] = await Promise.all([
    api("/api/health"),
    api("/api/summary"),
    api("/api/quality"),
    api("/api/workflow"),
    ...
    api("/api/pipeline"),
    api("/api/pipeline/daily-batch?limit=200&central=1"),
    api("/api/pipeline/inventory?limit=200"),
    api("/api/outbox"),
    api("/api/mailbox"),
    api("/api/campaigns"),
...
function renderOverview() {
  ...
  const task = state.operationTasks.find((item) => item.hsCode === planEntry.hsCode) || state.operationTasks[0] || null;
  ...
  const sendLimit = Math.min(Number(budgets.emailSendsDaily || budgets.emailSends || 20), Number(state.health?.delivery?.daily?.limit || 200));
      ...
      <section class="mission-console" aria-labelledby="missionTitle">
        <div class="mission-identity">
          ...
          <form class="mission-plan-form" id="overviewManagedPlanForm">
            <label for="overviewHsCode" id="missionTitle">HSCode 采集计划</label>
            <div class="mission-plan-input">
              <input class="input" id="overviewHsCode" name="hsCode" inputmode="numeric" pattern="[0-9]{4,10}" value="${escapeHtml(planEntry.hsCode || task?.hsCode || "")}" placeholder="输入4至10位HSCode" required />
              <button class="button primary" type="submit"><i data-lucide="play"></i>切换并启动托管采集</button>
            </div>
            <small>保存后服务器会停用旧采集任务，并按当前100家有效公司、200封邮件目标继续运行。</small>
          </form>
        ...
        <div class="mission-progress" aria-label="今日执行进度">
          <div class="daily-status-bar ${escapeHtml(dailyStatus.code)}" data-testid="daily-task-status">
            ...
            <span class="daily-status-copy"><strong>今日任务：${escapeHtml(dailyStatus.label)}</strong><small>${escapeHtml(dailyStatus.detail || "状态已同步")}</small></span>
          </div>
...
function renderWorkflow() {
  ...
  return `
    <div class="view-stack">
      ...
      <div class="callout"><i data-lucide="circle-stop"></i><div><strong>默认阻塞点：出站通道、自然反馈与联系人许可证据</strong><span>公开版不附带任何域名、邮箱或发送授权；使用者必须配置自己的服务并完成合规核验。</span></div></div>
    </div>`;
}
          ...
          <table>
            ...
            <tbody>${result.items.map((item) => `
              <tr>
                ...
                <td>${safeExternalUrl(item.linkedin) ? `<a class="button small" href="${escapeHtml(safeExternalUrl(item.linkedin))}" target="_blank" rel="noopener noreferrer"><i data-lucide="linkedin"></i>档案</a>` : "-"}</td>
      ...
      <section class="split-layout ops-layout">
        <div class="panel">
          ...
          <form class="panel-body editor" id="opsTaskForm">
            <div class="form-grid"><div class="field"><label for="opsHsCode">HSCode</label><input class="input" id="opsHsCode" name="hsCode" inputmode="numeric" pattern="[0-9]{4,10}" value="000000" required /></div><div class="field"><label for="opsDirection">方向</label><select class="select" id="opsDirection" name="direction"><option value="buyer">采购商</option><option value="supplier">供应商</option></select></div></div>
            ...
            <div class="form-grid"><div class="field"><label for="contactPageBudget">联系人分页动作/日</label><input class="input" id="contactPageBudget" name="contactPageBudget" type="number" min="1" max="300" value="200" /></div><div class="field"><label for="emailSendBudget">邮件发送/日</label><input class="input" id="emailSendBudget" name="emailSendBudget" type="number" min="1" max="200" value="200" /></div></div>
            <div class="field"><label for="opsAutomation">执行方式</label><select class="select" id="opsAutomation" name="automation"><option value="managed" selected>计划级托管</option><option value="supervised">逐批人工批准</option></select><small>托管目标为每日100家基础有效邮箱公司，每家公司最多2位联系人；保留格式、域名、去重、抑制名单、反馈熔断和每日200封硬上限。</small></div>
        ...
        <div class="panel">
          ...
          <div class="task-list">${tasks.length ? tasks.map((task) => `
            <button class="task-row ${task.id === selected?.id ? "active" : ""}" data-select-task="${escapeHtml(task.id)}" type="button">
              <span><strong>${escapeHtml(task.hsCode)}</strong><small>${task.direction === "buyer" ? "采购商" : "供应商"} · ${task.automation?.mode === "managed" ? "计划托管" : "人工逐批"} · ${escapeHtml(task.countries.join(", ") || "全部地区")}</small></span>
      ...
      ${selected ? `<section class="panel">
        ...
        <div class="panel-body">
          <div class="budget-grid">
            ...
            ${budgetMeter("邮件发送", state.health?.delivery?.daily?.used || 0, Math.min(budgets.emailSendsDaily || 20, state.health?.delivery?.daily?.limit || 200), Math.min(Math.max((budgets.emailSendsDaily || 20) - (state.health?.delivery?.daily?.used || 0), 0), state.health?.delivery?.daily?.remaining || 0))}
          ...
          <form class="ops-action-form" id="opsActionForm">
            ...
            <div class="action-strip">
              ...
              <button class="button" data-op-action="contact_page" type="button"><i data-lucide="product-open"></i>联系人分页</button>
        ...
        <div class="panel">
          ...
          <form class="panel-body editor" id="campaignForm">
            <section class="ai-builder" aria-labelledby="aiBuilderTitle">
              <div class="ai-builder-head"><div><h4 id="aiBuilderTitle">场景化 AI 邮件起草</h4><p>HSCode只用于内部产品匹配；正文不暴露海关数据来源。所有高风险卖点和配图均需人工批准。</p></div>${badge(aiRuntime.status)}</div>
              <div class="form-grid">
                ...
                <div class="field"><label for="hsCode">HSCode（仅内部匹配）</label><input class="input" id="hsCode" value="000000" required /></div>
              </div>
              ...
              <div class="field"><label for="productFocus">HSCode对应产品方向</label><textarea class="textarea compact" id="productFocus" required>Describe the product category using only verified internal data.</textarea></div>
              <div class="field"><label for="tradeContent">已核实的采购商业务证据</label><textarea class="textarea compact" id="tradeContent" required>Describe the buyer's relevant business using a traceable source.</textarea></div>
              <div class="field"><label for="companyBusiness">我方业务与可提供能力</label><textarea class="textarea compact" id="companyBusiness" required>Describe your company's verified products and services.</textarea></div>
              ...
              <div class="ai-builder-actions"><button class="button primary" id="aiDraftButton" type="button" ${aiReady ? "" : "disabled"}><i data-lucide="wand-sparkles"></i>生成候选草稿</button><span id="aiDraftStatus">${escapeHtml(aiRuntime.label)}</span></div>
              <div id="draftCandidates">${renderDraftCandidates()}</div>
            </section>
            <div class="field"><label for="campaignName">活动名称</label><input class="input" id="campaignName" name="name" value="Example buyer pilot" required /></div>
            <div class="field"><label for="campaignSubject">邮件主题</label><input class="input" id="campaignSubject" name="subject" value="A practical option for {{company}}" required /></div>
...
Your Company provides the verified products and services described in this campaign. {{company}} appears relevant based on the supplied buyer evidence.

We can review one current specification and return a clear, requirement-based response without a long back-and-forth.

Would you be open to sharing one current specification or RFQ for review? If another colleague manages supplier sourcing, I would appreciate an introduction.

Best regards,
Your Company</textarea></div>
            <div class="form-grid">
              <div class="field"><label for="campaignCountry">目标国家</label><select class="select" id="campaignCountry"><option value="" selected>全部国家</option>${state.options.countries.map((item) => `<option value="${escapeHtml(item)}">${escapeHtml(item)}</option>`).join("")}</select></div>
    ...
    <div class="view-stack sender-profile-view">
      ...
      <section class="sender-profile-layout">
        ...
        <aside class="sender-profile-aside">
          ...
          <section class="panel">
            <div class="panel-head"><h3>域名身份</h3><span class="badge neutral">由用户配置</span></div>
...
function renderMailbox() {
  ...
  return `
    <div class="view-stack">
      <div class="section-header"><div><h2>统一邮件中心</h2><p>集中查看你配置的邮箱；回复从原收件账号发出，批量发送门保持独立锁定。</p></div><div class="section-actions"><span class="badge neutral">${number(mailbox.counts?.synced)}/${number(mailbox.counts?.accounts)} 已同步</span><span class="badge neutral">更新 ${shortDateTime(mailbox.updatedAt)}</span></div></div>
...
function renderMailboxReplyCenter(mailbox) {
  ...
  return `<section class="panel mailbox-reply-center" data-testid="mailbox-reply-center">
    <div class="panel-head"><div><h3>回信处理台</h3><p>集中查看收到的邮件，按类型和处理状态分流；选择一封后可在右侧回复。</p></div><span class="badge ${pending ? "partial" : "complete"}">${number(pending)} 封待处理</span></div>
  ...
  return `
    <div class="view-stack">
      ...
      <section class="panel"><div class="panel-head"><h3>功能开关</h3></div><div class="table-shell"><table><thead><tr><th>能力</th><th>状态</th><th>说明</th></tr></thead><tbody>
        ...
        <tr><td>AI邮件起草</td><td>${badge(aiReady ? "complete" : "blocked")}</td><td>${aiReady ? `Responses API已配置，模型 ${escapeHtml(state.health.ai.model)}` : "OPENAI_API_KEY未配置"}</td></tr>
        <tr><td>邮件草稿与预览</td><td>${badge("complete")}</td><td>AI输出先进入人工可编辑草稿，不会自动发送</td></tr>
        <tr><td>SMTP连接</td><td>${badge(smtpReady ? "complete" : "blocked")}</td><td>${smtpReady ? "SMTP凭据完整，前端不可读取" : "公开版未加载任何SMTP凭据"}</td></tr>
  ...
  return `
    <div class="view-stack settings-view">
      ...
      <section class="dependency-grid">
        ...
        ${dependencyCard("confirm", "用户确认即可推进", "待你提供证据", "user-round-check", ["业务事实与卖点白名单", "每批发送的收件人、主题和退订策略", "联系人职位、采购证据与所选图片的人工复核"], "公开版不包含任何公司身份、域名、邮箱或图片；所有事实必须由使用者自行配置和核验。")}
        ${dependencyCard("external", "邮箱基础设施", "待用户配置", "key-round", ["配置自己的域名与 MX/SPF/DKIM/DMARC", "为自己的邮箱创建独立应用密码", "配置自己的 SMTP、IMAP 与反馈回调", "确认服务商允许预期的发送用途"], "示例配置不会连接任何维护者或第三方生产环境。")}
      </section>
      <section class="panel dependency-table-panel"><div class="panel-head"><div><h3>能力验收矩阵</h3><span class="panel-caption">状态是当前本地检测结果，不代表外部服务已经购买或联系人已经验证。</span></div></div><div class="table-shell"><table><thead><tr><th>能力</th><th>当前状态</th><th>为什么受限</th><th>完成证据</th></tr></thead><tbody>
        ...
        <tr><td>AI 草稿</td><td>${badge(aiRuntime.status)}</td><td>${escapeHtml(aiRuntime.label)}；输出仍需审核</td><td>最近请求时间、成功时间、错误与人工审核记录</td></tr>
        ...
        <tr><td>SMTP 连接与真实发送</td><td>${badge(sendingEnabled ? "partial" : "blocked")}</td><td>${escapeHtml(sendingEnabled ? deliverySummary : "公开版未配置任何邮箱或真实发送凭据；默认保持锁定")}</td><td>服务商许可、适用地区法律依据、退信/退订日志与逐批审计</td></tr>
...
2. 发件基础设施：配置你自己的域名、SMTP/IMAP账号、应用密码和每日额度。
3. 身份与域名：完成你自己的MX/SPF/DKIM/DMARC、MFA和服务商用途验收。
4. 公司身份：填写你自己的合法主体、对外名称、电话、官网和实体地址。
5. 合规配置：回复公司邮箱退订、抑制名单维护、退信/投诉回调和内部金丝雀。

无需购买、但必须由业务负责人确认：业务事实、卖点白名单、每批具体配图、收件人和退订策略。公开版不附带任何公司身份、官网或产品图片。

验收标准：企业邮箱只用于正常商务通信、收件与回复；营销触达必须使用允许该用途的出站产品，并保存明确许可、逐批审计与退订证据。管理员权限和每日200封上限不能替代产品规则或收件人许可。`;
...
async function loadLocalControls() {
  ...
  const fallback = [state.contactQuality, state.suppressions, { items: state.operationTasks }, { items: state.contactQueues }, state.pipeline, state.dailyBatch, state.outbox];
  ...
  const [contactQuality, suppressions, operations, contactQueues, pipeline, dailyBatch, outbox] = values;
...
async function exportLocalBackup() {
  try {
    ...
    downloadJson(`outreach-local-control-backup-${stamp}.json`, backup);
...
async function createOpsTask(event) {
  ...
  if (payload.automation.mode === "managed") {
    if (!window.confirm(`确认授权 HSCode ${payload.hsCode} 按计划自动采集、生成合规草稿并在全部安全门通过后发送？`)) return;
    payload.automation.confirm = `AUTHORIZE MANAGED ${payload.hsCode}`;
    ...
    const task = await api("/api/ops/tasks", {
      ...
      body: JSON.stringify({
        ...
        budgets: {
          ...
          emailSendsDaily: Number(budgets.emailSends || 200),
        },
        automation: {
          mode: "managed",
