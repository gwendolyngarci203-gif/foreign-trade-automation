...
def load_accounts():
    ...
    for account in accounts:
        ...
        if account.get("enabled", True) and "@" in address and password:
            cleaned.append({
                "address": address,
                "password": password,
                "imapHost": str(account.get("imapHost") or "imap.qiye.aliyun.com").strip(),
                "imapPort": int(account.get("imapPort") or 993),
                "smtpHost": str(account.get("smtpHost") or "smtp.qiye.aliyun.com").strip(),
...
def post_event(base_url, secret, event, account):
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-Feedback-Source": f"alimail-imap:{account}"[:80],
        },
        method="POST",
    )
...
def self_test():
    ...
    assert classify(reply, "dakingscc.cc") == {"type": "reply", "email": "buyer@example.com", "messageId": "m1@example"}
    automatic = email.message_from_string("From: buyer@example.com\nSubject: Away\nAuto-Submitted: auto-replied\n\n")
    assert classify(automatic, "dakingscc.cc")["type"] == "auto_reply"
    own = email.message_from_string("From: maggie1@dakingscc.cc\nSubject: Re: Quote\nIn-Reply-To: <m1@example>\n\n")
    assert classify(own, "dakingscc.cc") is None
    summary = mailbox_message(b"From: Buyer <buyer@example.com>\nTo: maggie1@dakingscc.cc\nSubject: Hello\nMessage-ID: <x@example>\n\nBody", "maggie1@dakingscc.cc", 7, "1")
    assert summary["from"]["address"] == "buyer@example.com" and summary["bodyText"] == "Body" and summary["unread"]
    print("SELF_TEST=OK")
