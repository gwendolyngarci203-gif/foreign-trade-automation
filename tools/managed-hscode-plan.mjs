...
export function validateManagedPlan(input) {
  ...
  if (!hsCodes.length) throw new Error("计划至少需要一个 HSCode");
  const emailSends = bounded(input.dailyBudgets?.emailSends, 20, 1, 200);
  ...
  const dailyBudgets = {
    ...
    emailSends,
    globalEmailHardCap: bounded(input.dailyBudgets?.globalEmailHardCap, 200, 1, 200),
  };
...
export async function registerManagedPlan(plan, apiBase) {
  ...
  const results = [];
  for (const item of plan.hsCodes) {
    const prior = existing.items.find((task) => (
      task.hsCode === item.hsCode
    ...
    if (prior) {
      await request(apiBase, `/api/ops/tasks/${prior.id}`, {
        ...
        body: JSON.stringify({
          ...
          confirm: `AUTHORIZE MANAGED ${item.hsCode}`,
        }),
      });
